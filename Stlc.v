...
Inductive substi (s : tm) (x : string) : tm -> tm -> Prop :=
  ...
  | s_var2 (y : string): x <> y -> substi s x (tm_var y) (tm_var y)
  | s_abs1 (n : tm) (T : ty) : substi s x <{\x:T, n}> <{\x:T, n}>
  | s_abs2 (t t' : tm) (T : ty) (y : string) (H : substi s x t t') : 
      x <> y -> substi s x <{\y:T, t}>  <{\y:T, t'}>
...
Proof.
  ...
  - intros. generalize dependent t'. 
    induction t as [x'|t1 H1 t2 H2|y T t1 H| | |t1 H1 t2 H2 t3 H3].
    + intros. simpl in H. destruct (eqb_string x x') eqn: EQB.
      * apply eqb_string_true_iff in EQB. rewrite <- H. rewrite <- EQB. apply s_var1.
      * apply eqb_string_false_iff in EQB. rewrite <- H. apply s_var2. apply EQB.
    + intros. simpl in H. rewrite <- H. apply s_app.
      * apply H1. reflexivity.
      * apply H2. reflexivity.
    + intros t' H'. simpl in H'. destruct (eqb_string x y) eqn: EQB.
      * apply eqb_string_true_iff in EQB. rewrite <- H'. rewrite <- EQB. apply s_abs1.
      * apply eqb_string_false_iff in EQB. rewrite <- H'. apply s_abs2.
        apply H. reflexivity. apply EQB.
    + intros t' H. simpl in H. rewrite <- H. apply s_true.
    + intros t' H. simpl in H. rewrite <- H. apply s_false.
    + intros. simpl in H. rewrite <- H. apply s_if.
      * apply H1. reflexivity.
      * apply H2. reflexivity.
      * apply H3. reflexivity.
  - intros. 
    induction H as [|y|n T|t t' T y H|t1 t1' t2 t2' H1 IH1 H2 IH2| | |t1 t1' t2 t2' t3 t3' H1 IH1 H2 IH2 H3 IH3].
    + simpl. rewrite <- eqb_string_refl. reflexivity.
    + simpl. apply eqb_string_false_iff in H. rewrite H. reflexivity.
    + simpl. rewrite <- eqb_string_refl. reflexivity.
    + simpl. apply eqb_string_false_iff in H0. rewrite H0. rewrite IHH. reflexivity.
    + simpl. rewrite IH1, IH2. reflexivity.
    + reflexivity.
    + reflexivity.
    + simpl. rewrite IH1,IH2,IH3. reflexivity. 
Qed.
...
(* 为什么app1和app2不同是因为对于函数，首先规约函数，再规约值 小步语义从左到右 *)
...
Proof.
  ...
  eapply multi_step.
    simpl. apply ST_AppAbs. auto. simpl.
  apply multi_refl.
Qed.
...
(* markcty: empty is from partial map, 
  use partial as a tool to build context,
  Gamma is a context !!! *)

Example typing_example_1 :
  empty |- \x:Bool, x \in (Bool -> Bool).
Proof.
  apply T_Abs. apply T_Var. simpl. reflexivity.  Qed.
...
Proof.
  ...
  Unset Printing Notations.
  apply T_Abs.
  apply T_App with Ty_Bool. apply T_Var. Unset Printing Notations. apply update_eq.
  apply T_App with Ty_Bool. apply T_Var. apply update_eq.
  apply T_Var. Unset Printing Notations. apply update_neq.
...
Proof.
  ...
  apply T_App with Ty_Bool. apply T_Var. apply update_neq. intros. discriminate.
  apply T_App with Ty_Bool. apply T_Var. apply update_neq. intros. discriminate.
...
Example typing_nonexample_3 :
  ~ (exists S T,
        empty |-
          \x:S, x x \in T).
Proof.
  intros Hc. destruct Hc as [S [T]].
  inversion H. subst. clear H.
  inversion H5. subst. clear H5.
  inversion H4. subst. clear H4.
  inversion H2. subst. rewrite H3 in H1.
  inversion H1. clear H2 H1 H3 S.
  (* infinite nest type *)
  induction T2.
  - discriminate.
  - inversion H0. subst. apply IHT2_1. apply H1.
Qed.
End STLC.
