...
Fixpoint is_recr_call (fname : string) (t : tm) :=
  match t with
  | (tm_app t' _) => is_recr_call fname t'
  | (tm_var f) => eqb_string fname f
  | _ => false
  end.

(* replaces all recursive calls return (call_list, expression, recursive call times) *)
Fixpoint replace_recr_calls (fname : string) (t : tm) (call_list : tm) (calls : nat) :=
  match t with
  (* exp tree(inductively defined) *)
  | <{m1 + m2}> =>
    match (replace_recr_calls fname m1 call_list calls) with 
    | (call_list, (lt, lcalls)) => 
      match (replace_recr_calls fname m2 call_list lcalls) with
      | (call_list, (rt, rcalls)) => (call_list,(<{lt + rt}>, rcalls))
      end
    end
  | <{m1 :: m2}> =>
    match (replace_recr_calls fname m1 call_list calls) with 
  ...
  (* recursive call *)
  | (tm_app _ _) => 
    if (is_recr_call fname t) 
    then (<{t::call_list}>, (tm_var (create_new_name calls), calls + 1))
    else (call_list,(t,calls))
  (* reach leaf *)
  | (tm_const n) => (call_list, (t, calls))
  | (tm_var s) => (call_list,(t,calls))
  | _ => (tm_invalid,(tm_invalid, 100))
  end.

Fixpoint combine (call_list : tm) (exp : tm) (calls : nat):=
  match call_list with
  | <{nil}> => tm_app k exp
  | <{call :: l'}> => 
    let name := create_new_name calls in
    let tail := combine l' exp (calls + 1) in
    <{call (\name, tail)}>
  | _ => tm_invalid
  end.

Definition to_CPS (triple : tm * (tm * nat)) :=
  match triple with
  | (call_list,(exp,_)) => (combine call_list exp 1)
  (* | _ => tm_invalid *)
  end.
...
Definition to_tail(t:tm) := 
  ...
  | tm_fix (tm_abs fname
            (tm_abs l
              (tm_lcase _ init_t _ _ exp))) =>
                let cont :=<{fix 
                            (\f, \l, \k,
                                case l of
                                | nil => k init_t
                                | x::l' => {to_CPS (replace_recr_calls fname exp <{nil}> 1)}
                            )}> in
                <{\l, cont l <{\s,s}>}>
  | _ => tm_invalid
  end.
...
Proof.
  unfold not_tail_nested1,list. simpl. simpl.
  normalize.
Qed.
