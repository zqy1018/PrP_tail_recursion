...
(** **** Exercise: 1 star, standard, optional (halve_fix) 
    ...
    halve = (\f. \x:Nat,if (test x=0) then 0
                        else if (test (pred x)=0) then 0
...
(** **** Exercise: 1 star, standard, optional (fact_steps) 
    ...
    1 * ((\x. test x=0 then 1 else x * (f (pred x))) 0)

    1 * (test 0=0 then 1 else 0 * (f (pred 0)))
...
Fixpoint subst (x : string) (s : tm) (t : tm) : tm :=
  ...
  (* let *)
  | <{let y = t1 in t2}> =>
    tm_let y <{[x:=s] t1}> (if eqb_string x y then t2 else <{[x:=s] t2}>)
    (* markcty: why x in t1 should be directly substituted? t1 is evaluated in the original environment. *)
  (* fix *)
  | <{fix t'}> => <{fix ([x:=s] t')}>
  end
...
Proof.
  ...
  (* by hand
    eapply multi_step.
    {
      apply ST_Snd1. apply ST_FstPair. apply v_pair; apply v_nat. apply v_nat.
    }
    eapply multi_step.
    {
...
Proof with eauto.
  ...
  - (* T_Pair *)
    ...
    + (* t1 is a value *)
      ...
      (* t2 is a value(auto) *)
      * (* t2 steps *)
        right. destruct H0 as [t2']. exists <{(t1, t2')}>.
        apply ST_Pair2...
    + (* t1 steps *)
      right. destruct H as [t1']. exists <{ (t1', t2)}>.
      apply ST_Pair1...
  - (* T_Fst *)
    ...
    + (* t0 steps *)
      destruct H as [t0']. exists <{t0'.fst}>...
  - (* T_Snd *)
    right. destruct IHHt; subst...
    + (* t0 is a value *)
      destruct H; try solve_by_invert. exists v2...
    + (* t0 stpes *)
      destruct H as [t0']. exists <{t0'.snd}>...
  - (* T_Let *)
  ...
  - (* T_Fix *)
    ...
    + (* t1 is a value *)
      destruct H; try solve_by_invert. 
      rename x0 into xf.
      exists <{[xf:=fix(\xf:T2, t1)] t1}>...
    + (* t1 steps *) 
      destruct H as [t1']. exists <{fix t1'}>...
Qed.
  ...
  - (* tm_lcase *)
    ...
    + (* x<>y1 *)
      ...
      * (* x<>y2 *)
        ...
        assumption.
  - (* tm_let: the definition on subst in let is important and a little bit hack *)
    rename T into T2.
    destruct (eqb_stringP x s); subst; apply T_Let with T1...
    + (* x = s *)
...
Proof with eauto.
  ...
  - (* T_Fst *)
    rename t' into t1. inversion HT; subst...
  - (* T_Snd *)
    rename t' into t2. inversion HT; subst...
  - (* T_Let *)
    rename x0 into x.
    apply substitution_preserves_typing with T1...
  - (* T_Fix *)
    rename t0 into t1, T0 into T.
    apply substitution_preserves_typing with (T1)...
    inversion HT; subst...
Qed.
