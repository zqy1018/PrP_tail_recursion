...
From TRO Require Import Types.
From TRO Require Import Stlc.
...
Proof.
  ...
  (*
    ...
    markcty: ';' applies auto to every subgoal HVal generates,
    and leaves the subgoals unsolved
  *)
  destruct HVal. 
  - auto. inversion HT.
  - auto.
...
Proof.
  ...
  destruct HVal.
  - inversion HT. subst. exists x0, t1. reflexivity.
  - inversion HT.
  - inversion HT.  
Qed.
...
Proof.
  ...
  - inversion Ht;subst. 
    specialize (IHt1 (Ty_Arrow T2 T)). specialize (IHt2 T2).
    apply IHt1 in H2 as IH2. apply IHt2 in H4 as IH4.
    destruct IH2...
    + destruct IH4...
      * (* t1 and t2 are both value *)
        right. apply canonical_forms_fun in H2; [|assumption].
        destruct H2 as [x [tt]]. rewrite H1. exists <{ [x:=t2]tt}>.
        apply ST_AppAbs;assumption.
      * (* t1 is value *)
        right. destruct H0 as [t2'].
        exists <{t1 t2'}>. apply ST_App2;assumption.
    + right. destruct H as [t1'].
      exists <{t1' t2}>. apply ST_App1;assumption.
  - inversion Ht; subst. right.
    apply IHt1 in H3 as H3'.
    destruct H3'.
    + apply canonical_forms_bool in H3; [|assumption].
      destruct H3.
      * exists t2. rewrite H0. apply ST_IfTrue.
      * exists t3. rewrite H0. apply ST_IfFalse.
    + destruct H as [t1']. exists <{ if t1' then t2 else t3}>.
...
Proof.
  ...
  - (* var *)
    rename s into y. destruct (eqb_stringP x y); subst. (* markcty: rename tactic *)
    + (* x=y *)
...
Proof.
  ...
  - rename x0 into t. destruct (eqb_stringP x t).
    + rewrite G in H. rewrite e in H. rewrite update_eq in H.
      injection H as H. rewrite <- H. apply weakening_empty; assumption.
    + apply T_Var. rewrite G in H. rewrite update_neq in H; [|assumption].
      apply H.
  - rename t1 into t'. rename x0 into x'. destruct (eqb_stringP x x').
    + apply T_Abs. rewrite G in Ht. rewrite e in Ht. rewrite update_shadow in Ht. assumption.
    + apply T_Abs. apply IHHt. rewrite G. apply update_permute. apply n.
Qed. 
...
Proof.
  ...
  - apply progress in Hhas_type. destruct Hhas_type.
    + apply Hnot_val,H.
    + apply Hnf,H.
  - apply IHHmulti; [|assumption|assumption].
...
Theorem unique_types : forall Gamma e T T',
  ...
  T = T'.
Proof.
  intros Gamma e T T' G G'.
  generalize dependent T'.
  induction G; 
    intros T' G'; inversion G'; subst; 
    try solve [reflexivity].
  - rewrite H2 in H. injection H as H. symmetry. apply H.
  - apply IHG in H4. rewrite H4. reflexivity.
  - apply IHG1 in H2. apply IHG2 in H4. rewrite H4 in H2.
    injection H2 as H2. assumption.
  - inversion G'; subst. apply IHG2 in H5. assumption.
Qed.
...
Proof.
  ...
  rename y0 into y.
  inversion H1; subst.
  apply IHappears_free_in in H7. destruct H7 as [T'].
  exists T'. rewrite update_neq in H2; assumption.
Qed.
...
Corollary typable_empty__closed : forall t T,
    ...
    closed t.
Proof.
  intros. unfold closed. intros x Hc.
  assert (exists (T':ty), empty x = Some T').
  apply (free_in_context x t T empty); assumption.
...
Proof.
  ...
  - rename x0 into x. apply T_Var. rewrite <- H. 
    symmetry. apply H0. apply afi_var.
  - rename x0 into x. apply T_Abs. apply IHhas_type.
    intros. destruct (eqb_stringP x x0).
    + rewrite e. rewrite update_eq. rewrite update_eq. reflexivity.
...
(** **** Exercise: 1 star, standard (progress_preservation_statement) 
    ...
    Progress: The progress theorem tells us that closed, 
    well-typed terms are not stuck: either a well-typed term is a value, 
    or it can take a reduction step.

    Preservation: If a closed term t has type T and takes a step to t', 
    then t' is also a closed term with type T. 
    In other words, the small-step reduction relation preserves types.
...
and the following typing rule:
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
        become false
        every term can step into zap or follow the initial rule
      - Progress
        becomes false
        zap is not a value but can't step further
      - Preservation
        remains true
        every term has type [zap] now
*)
...
(** **** Exercise: 2 stars, standard (stlc_variation2) 
    ...
    false, give a counterexample.
      ...
      - Preservation
        becomes false
        \x:A, x has Type A->A
        It steps into foo, which has no type
*)
...
(** **** Exercise: 2 stars, standard (stlc_variation3) 
    ...
    false, give a counterexample.
      ...
      - Progress
        becomes false
        <{(\x:T2, t1) v2}> t0 is not a value
        it can's step either.
      - Preservation
        remains true
*)
...
(** **** Exercise: 2 stars, standard, optional (stlc_variation4) 
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
        becomes false.
        if true then t1 else t2 can step into both t1 and true
      - Progress
        remains true
      - Preservation
        becomes false
        t1: T, t2: T
        (if true then t1 else t2) steps into true: Ty_bool
        or stpes into t1: T
*)
(** [] *)
...
(** **** Exercise: 2 stars, standard, optional (stlc_variation5) 
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
        remains true
        T_FunnyApp situation can't be evalutated by normal rules
      - Progress
        t1: Bool -> Bool -> Bool
        t2: Bool
        t3: Bool
        t4: Bool
        if t1 t2 then t3 else t4 has type Bool
        But it can't step and is not a value
      - Preservation
        (\x:Bool, \y:Bool x) y can either take a normal step, leading to a type Bool->Bool
        or take the funny step, leading to a type Bool
*)
...
(** **** Exercise: 2 stars, standard, optional (stlc_variation6) 
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
        remains true
      - Progress
        if (t1 t2) then t1 else t2 is not a value and can't take a step
      - Preservation
        remains true
*)
(** [] *)
...
(** **** Exercise: 2 stars, standard, optional (stlc_variation7) 
    ...
    false, give a counterexample.
      ...
      - Progress
        becomes false
        if (\x:Bool,t) then t1 else t2 can't step and is not a value
      - Preservation
        same as variation5
*)
(** [] *)
