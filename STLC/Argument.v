...
match fuel with | 0 => func_body | S fuel' =>
  ...
  | app t1 t2 =>
       ...
       then match (find_func func_name t1 - 1), find_func func_name t2 with
       | O, O =>
            app k (continuation func_body) (*impossible*)
       | S n1, O =>
       ...
       | S n1, S n2 =>
            CPS k func_name t1
            (fun res1 : tm => CPS k func_name t2
            (fun res2 : tm => continuation (app res1 t2)) (naming + (S n1)) fuel') naming fuel'
       end
...
Compute (CPS (var k) "F"
<{ F (F a (b - 1) c - 2) (F (a - 1) (b - 1) c - 1) }>
(fun res : tm => tplus 1 res) 1 999).
